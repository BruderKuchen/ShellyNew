...
export default function App() {
  ...
  const handleLogin = async (token) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setUser({ role });
        // direkt nach Login einmal Status holen
        fetchStatus();
        return;
      }
    } catch {}
  ...
  const fetchStatus = async () => {
    ...
    // im Fehlerfall Dummy-Status (Offline)
    setStatus({ state: "closed", temp: 0, battery: 0 });
  };
  ...
  return (
    <div className="min-h-screen bg-blue-50">
        ...
        <div className="bg-white rounded shadow p-6">
          ...
          {activeTab==="logs" && <>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={fetchHistory} className="px-4 py-2 bg-blue-500 text-white rounded">Refresh</button>
            </div>
          ...
          {activeTab==="users" && <>
            ...
            <form onSubmit={handleCreateUser} className="space-y-2 mb-6">
              <input className="border p-2 rounded w-full" placeholder="Username" required value={newUser.username} onChange={e=>setNewUser({...newUser,username:e.target.value})}/>
              <input className="border p-2 rounded w-full" placeholder="Password" type="password" required value={newUser.password} onChange={e=>setNewUser({...newUser,password:e.target.value})}/>
              <select className="border p-2 rounded w-full" value={newUser.role} onChange={e=>setNewUser({...newUser,role:e.target.value})}>
            ...
            <table className="w-full bg-white rounded shadow divide-y">
              <thead><tr className="bg-gray-100"><th className="p-2">Username</th><th className="p-2">Role</th><th className="p-2">Actions</th></tr></thead>
              <tbody>
                {users.map(u=>(
                  <tr key={u.id} className="hover:bg-gray-50">
                    <td className="p-2">{u.username}</td>
                    <td className="p-2 capitalize">{u.role}</td>
                    <td className="p-2">{u.role!=="admin" && <button onClick={()=>handleDeleteUser(u.id)}><Trash2 size={16}/></button>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {/* Floating Help */}
      <button onClick={()=>setShowHelpPopup(true)} className="fixed bottom-6 right-6 bg-blue-500 text-white p-4 rounded-full shadow-lg hover:bg-blue-600">
        <HelpCircle size={24}/>
      </button>
